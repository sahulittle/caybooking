...
const AdminRoles = () => {
  ...
  const handlePermissionChange = (perm) => {
    if (currentAdmin.permissions.includes(perm)) {
      setCurrentAdmin({
        ...currentAdmin,
        permissions: currentAdmin.permissions.filter((p) => p !== perm),
      });
    } else {
      setCurrentAdmin({
        ...currentAdmin,
        permissions: [...currentAdmin.permissions, perm],
      });
    }
  };
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="min-w-0">
          ...
          <p className="text-sm text-gray-500 mt-1">
            Manage admin access levels and roles
          </p>
        </div>

        <button
          onClick={() => openModal()}
          className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors font-semibold text-sm shadow-sm"
        >
