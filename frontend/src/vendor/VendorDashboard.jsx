...
const VendorDashboard = () => {
  ...
  const fetchBookings = async () => {
    try {
      ...
      const mapped = (res.data.bookings || []).map((b) => ({
        ...
        service: b.service?.title || b.service || "Service",
        date: b.bookingDate
  ...
  useEffect(() => {
    ...
    // setup socket to receive live updates
        ...
        <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-1 bg-white rounded-2xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow duration-300 flex flex-col h-full">
              <div className="flex justify-between items-center mb-6">
                ...
                <span className="bg-[#7AB2B2]/60 text-[#09637E] border border-[#7AB2B2] text-xs font-bold px-3 py-1 rounded-full">
                  3 Urgent
                </span>
              </div>
              ...
              <button
                onClick={() => navigate("/vendor/bookings")}
                className="w-full mt-6 py-3 border border-dashed border-gray-200 rounded-xl text-sm font-bold text-gray-500 hover:text-[#088395] hover:border-blue-200 hover:bg-blue-50 transition-all flex items-center justify-center gap-2"
              >
                View All Actions
              </button>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow duration-300">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center">
              ...
              <button
                onClick={() => navigate("/vendor/bookings")}
                className="text-[#088395] font-bold text-sm hover:bg-blue-50 px-4 py-2 rounded-lg transition-colors"
              >
                See All
              </button>
            </div>
                  ...
                  {recentBookings.map((booking) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`px-3 py-1 inline-flex text-xs leading-5 font-bold rounded-full ${getStatusStyle(booking.status)}`}
                        >
                          {booking.status}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <button
                          onClick={() => navigate("/vendor/bookings")}
                          className="text-gray-400 hover:text-[#088395] p-2 rounded-lg hover:bg-blue-50 transition-all"
                          title="View Details"
                        >
