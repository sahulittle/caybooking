...
const Subscription = () => {
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      try {
        const { subscriptionAPI } = await import("../../api/apiClient");
        ...
        setPlans(
          (res.data?.plans || []).map((p) => ({
            ...p,
            btnText:
              p.btnText || (p.price > 0 ? "Subscribe Now" : "Start Free Trial"),
            color: p.color || "border-gray-200",
            buttonStyle:
              p.buttonStyle ||
              (p.price > 0
        ...
        <div className="w-full mx-auto">
          <div className="text-center max-w-3xl mx-auto mb-16">
            ...
            <p className="text-xl text-gray-500">
              Unlock the full potential of CaymanMaintenance with our flexible
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
            {plans.map((plan) => (
              ...
              >
                ...
                <div className="mb-6">
                  <div
                    className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-4 ${plan.isGold ? "bg-gradient-to-br from-amber-400 to-orange-500 text-white" : plan.popular ? "bg-blue-600" : "bg-blue-50"}`}
                  >
                    {plan.icon}
                  </div>
                ...
                <ul className="space-y-4 mb-8">
                  {plan.features.map((feature, i) => (
                    <li key={i} className="flex items-start gap-3">
                      <div
                        className={`mt-0.5 p-0.5 rounded-full ${plan.isGold ? "bg-amber-100 text-amber-600" : "bg-green-100 text-green-600"}`}
                      >
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
              ...
              <div className="p-6 space-y-6">
                ...
                <p className="text-xs text-center text-gray-400 flex items-center justify-center gap-1">
                  <Shield className="w-3 h-3" /> Secure 256-bit SSL Encrypted
                  Payment
                </p>
              </div>
            </div>
