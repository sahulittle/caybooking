...
const Services = () => {
  ...
  const [services, setServices] = useState([
    { id: 1, name: 'AC Repair', price: 120, description: 'Complete AC repair and servicing.', image: 'https://images.unsplash.com/photo-1621905251189-08b45d6a269e?w=200&q=80' },
    { id: 2, name: 'Plumbing', price: 80, description: 'Fixing leaks and pipe installation.', image: 'https://images.unsplash.com/photo-1504328345606-18bbc8c9d7d1?w=200&q=80' },
  ])
  const [newService, setNewService] = useState({ name: '', price: '', description: '', image: null })
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    if (editingId) {
      setServices(services.map(s => s.id === editingId ? { ...s, ...newService, price: Number(newService.price) } : s))
      setEditingId(null)
    } else {
  ...
  const handleEdit = (service) => {
    ...
    setNewService({ name: service.name, price: service.price, description: service.description, image: service.image })
    setIsModalOpen(true)
  }
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {services.map((service) => (
              <div key={service.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow group">
                <div className="h-48 overflow-hidden relative">
                  ...
                  <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => handleEdit(service)} className="p-2 bg-white/90 rounded-full text-[#088395] hover:bg-white shadow-sm"><Edit2 className="w-4 h-4" /></button>
                    <button onClick={() => handleDelete(service.id)} className="p-2 bg-white/90 rounded-full text-red-500 hover:bg-white shadow-sm"><Trash2 className="w-4 h-4" /></button>
                  </div>
                </div>
                <div className="p-5">
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="font-bold text-lg text-gray-900">{service.name}</h3>
                    <span className="font-bold text-[#088395] bg-[#088395]/20 px-2 py-1 rounded-lg text-sm">${service.price}</span>
                  </div>
              ...
              <div>
                ...
                <div className="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center hover:bg-gray-50 transition-colors relative">
                  ...
                  <div className="flex flex-col items-center pointer-events-none">
                    ...
                    <p className="text-sm text-gray-500 font-medium">{newService.image ? 'Image Selected' : 'Click to upload image'}</p>
                  </div>
                </div>
                {newService.image && <p className="text-xs text-green-600 mt-2 font-semibold text-center">Image ready to upload</p>}
              </div>

              <button type="submit" className="w-full py-3 bg-[#088395] text-white font-bold rounded-xl hover:bg-[#088395]/70 transition-colors mt-2">
