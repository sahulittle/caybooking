...
import { Check, Star, Fan, Zap, Droplet, Home as HomeIcon,Snowflake,BrushCleaning,Wrench,Bug,Sparkles} from 'lucide-react';
    ...
    <div className="font-sans text-gray-700 bg-white">
      ...
      <section className="relative px-8 pt-40 pb-48 text-center overflow-hidden">
        {heroImages.map((img, index) => (
          ...
          >
            <img src={img} alt="Hero Background" className="w-full h-full object-fill" />
        ...
        <div className="max-w-4xl mx-auto relative z-10">
          ...
          <div className="flex justify-center gap-6">
            <button onClick={() => navigate('/bookingpage')} className="py-3 px-6 md:py-4 md:px-8 text-base rounded-full cursor-pointer font-semibold shadow-md transition-transform hover:scale-105 bg-gradient-to-r from-blue-600 to-blue-800 text-white shadow-lg shadow-blue-500/50">Book Now</button>
      ...
      <section className="py-24 px-8 max-w-7xl mx-auto bg-gray-50 rounded-2xl">
        <h2 className="text-center text-3xl md:text-4xl font-extrabold text-gray-900 mb-16">How It Works</h2>
        <div className="flex flex-col md:flex-row justify-around gap-8 text-center">
          {howItWorksSteps.map(step => (
            <div key={step.number} className="flex-1">
              <div className="w-12 h-12 rounded-full bg-gradient-to-r from-blue-600 to-blue-800 text-white flex items-center justify-center text-2xl font-bold mx-auto mb-4">{step.number}</div>
      ...
      {/* 5. Why Choose Us */}
      <section className="py-24 px-8 max-w-7xl mx-auto">
        <h2 className="text-center text-3xl md:text-4xl font-extrabold text-gray-900 mb-16">Why Choose Us</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {whyChooseUsPoints.map(point => (
            <div key={point.title} className="bg-white p-8 rounded-xl text-center">
              <div className="flex justify-center"><CheckmarkIcon /></div>
              <h3 className="text-xl font-semibold text-gray-900 mt-4 mb-2">{point.title}</h3>
      ...
      <footer className="bg-gray-900 text-gray-300 pt-16 px-8">
        <div className="max-w-7xl mx-auto flex justify-between flex-wrap gap-8 pb-12">
          <div className="flex-1 min-w-[200px]">
            <h3 className="text-lg font-semibold text-white mb-4">Caymantainane</h3>
