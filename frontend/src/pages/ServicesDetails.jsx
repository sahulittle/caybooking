...
import { allServicesData } from '../pages/servicesData'
...
const ServicesDetails = () => {
  ...
  const service = allServicesData.find(s => s.id === parseInt(serviceId))

  if (!service) {
    return (
      <div className="pt-32 text-center">
        <h1 className="text-2xl font-bold">Service not found</h1>
        <Link to="/service" className="text-blue-600 hover:underline mt-4 inline-block">Go back to services</Link>
      </div>
    )
  }
  ...
  return (
    <div className="bg-gray-50 min-h-screen font-sans pt-5 pb-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Breadcrumb */}
        <div className="mb-6">
          <Link to="/service" className="text-blue-600 hover:text-blue-700 font-medium flex items-center gap-2 transition-colors">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
              ...
              <div className="h-64 sm:h-80 w-full relative">
                ...
                <div className="absolute bottom-0 left-0 p-6 sm:p-8 text-white w-full">
                  ...
                  <div className="flex items-center gap-3 text-sm sm:text-base">
                    ...
                    <span className="opacity-90">{service.reviews} verified reviews</span>
                  </div>
                </div>
              </div>
              <div className="p-6 sm:p-8">
                <h2 className="text-xl font-bold text-gray-900 mb-3">About the Service</h2>
            ...
            <div className="bg-white rounded-2xl shadow-sm p-6 sm:p-8 border border-gray-100">
              <h2 className="text-xl font-bold text-gray-900 mb-6">Select a Service Plan</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {service.plans.map((plan) => (
                  ...
                  >
                    ...
                    <p className="text-gray-500 text-sm leading-snug">{plan.desc}</p>
                  </div>
                ))}
              </div>
            </div>
              ...
              <ul className="grid grid-cols-1 sm:grid-cols-2 gap-y-3 gap-x-6">
                {service.features.map((feature, idx) => (
                  <li key={idx} className="flex items-center text-gray-700">
                    <span className="w-6 h-6 rounded-full bg-green-100 text-green-600 flex items-center justify-center mr-3 flex-shrink-0 text-xs font-bold">✓</span>
                    {feature}
                  </li>
                ))}
              </ul>
                    ...
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-yellow-400 text-sm flex gap-0.5">{'★'.repeat(review.rating)}</div>
                        </div>
                      </div>
            ...
            <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-6 sticky top-28">
              ...
              {!selectedPlan ? (
                <div className="text-center py-10 px-4 text-gray-500 bg-gray-50 rounded-xl border border-dashed border-gray-300">
                  <p className="text-sm">Please select a service plan from the left to proceed with booking.</p>
                </div>
              ) : (
                <div className="space-y-4 animate-in fade-in zoom-in duration-300">
                  ...
                  <div className="flex justify-between items-center py-1">
                    ...
                    <span className="font-medium text-gray-900 text-sm">$5.00</span>
                  </div>

                  <div className="flex justify-between items-center pt-4 border-t border-gray-100 mt-2">
                    <span className="text-lg font-bold text-gray-900">Total</span>
                    <span className="text-3xl font-extrabold text-blue-600">${selectedPlan.price + 5}</span>
                  </div>
