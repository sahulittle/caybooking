...
const BookingPage = () => {
  ...
  const isStepValid = () => {
    ...
    if (currentStep === 2) return formData.address && formData.city && formData.zip
    return true
  }
  ...
  const handleConfirm = async () => {
    if (!formData.date || !formData.time || !formData.address || !formData.city || !formData.zip) {
    ...
    try {
      await sendMaintenanceRequest(payload)
      toast.success('Booking confirmed! We sent your request to our team.')
      navigate('/')
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-5 pb-12 font-sans">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Breadcrumb */}
        <div className="mb-8">
          <Link to="/service" className="text-gray-500 hover:text-blue-600 transition-colors flex items-center gap-2 text-sm font-medium">
              ...
              <div className="flex justify-between items-center relative">
                ...
                {steps.map((step) => (
                  <div key={step.id} className="relative z-10 flex flex-col items-center gap-2 bg-white px-2">
                    <div
                      className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm transition-all duration-300 border-2 ${currentStep >= step.id
                    ...
                    <span className={`text-xs font-semibold uppercase tracking-wider ${currentStep >= step.id ? 'text-blue-600' : 'text-gray-400'}`}>
                      {step.title}
                    </span>
                  </div>
                ))}
              ...
              {currentStep === 1 && (
                <div className="space-y-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        <select
                          ...
                          className="w-full p-4 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all bg-gray-50 appearance-none pr-10"
                        >
                          {service.plans && service.plans.map(p => (
                  ...
                  <div className="space-y-4">
                    <label className="block text-sm font-semibold text-gray-700">Pick a Date</label>
                    <input
                      type="date"
                      name="date"
                      value={formData.date}
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      {timeSlots.map(slot => (
                        <button
                          ...
                          className={`py-3 px-2 rounded-xl text-sm font-medium border-2 transition-all duration-200 ${formData.time === slot
              ...
              {currentStep === 3 && (
                <div className="space-y-6">
                      ...
                      <>
                        ...
                        <div className="flex gap-4 items-start">
                          ...
                          <div>
                            ...
                            <p className="text-gray-600 text-sm italic">"{formData.notes}"</p>
                          </div>
                        </div>
                      </>
                    )}
                  ...
                  <div className="flex items-center gap-2 text-sm text-gray-500 bg-gray-50 p-4 rounded-xl">
                    <input type="checkbox" id="terms" className="w-4 h-4 rounded text-blue-600 focus:ring-blue-500 border-gray-300" defaultChecked />
                    <label htmlFor="terms">I agree to the <span className="text-blue-600 cursor-pointer hover:underline">Terms of Service</span> and <span className="text-blue-600 cursor-pointer hover:underline">Privacy Policy</span>.</label>
                  </div>
                </div>
              )}
              ...
              <div className="flex justify-between pt-8 mt-4 border-t border-gray-100">
                ...
                {currentStep < 3 ? (
                  <button
                    ...
                    className={`px-8 py-3 rounded-xl font-bold text-white transition-all transform hover:-translate-y-0.5 shadow-lg ${isStepValid()
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-6 sticky top-28">
              ...
              <div className="flex gap-4 mb-6">
                <img src={service.image} alt={service.title} className="w-20 h-20 rounded-lg object-cover shadow-sm" />
                <div>
              ...
              <div className="bg-gray-50 rounded-xl p-4 text-xs text-gray-500 flex items-start gap-2">
                <svg className="w-5 h-5 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
