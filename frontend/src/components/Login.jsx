...
const Login = () => {
  ...
  const handleLogin = (e) => {
    ...
    if (email === 'admin@gmail.com' && password === '123456') {
      ...
      return;
    } else if (email === 'vendor@gmail.com' && password === '123456') {
      const user = { name: 'Vendor', email, role: 'vendor' };
      localStorage.setItem('user', JSON.stringify(user));
      window.dispatchEvent(new Event('authStateChange'));
      ...
      const user = { name: 'User', email, role: 'user' };
      localStorage.setItem('user', JSON.stringify(user));
      window.dispatchEvent(new Event('authStateChange'));
    ...
    const user = { name, email, role: isVendor ? 'vendor' : 'user' };
    localStorage.setItem('user', JSON.stringify(user));
    window.dispatchEvent(new Event('authStateChange'));
        ...
        <div className="flex-1 flex items-center justify-center p-8 md:p-12">
          <div className="w-full max-w-[380px]">
            ...
            <form onSubmit={handleLogin}>
              ...
              <button type="submit" className="w-full py-3.5 border-none rounded-lg bg-gradient-to-br from-indigo-600 to-purple-700 text-white text-base font-semibold cursor-pointer mt-4 shadow-lg shadow-indigo-500/40 hover:-translate-y-1 transition-transform duration-300">Log In</button>
            </form>
            ...
            <p className="text-center mt-8 text-gray-500 text-sm">
              Don't have an account? <Link to="/signup" className="text-indigo-600 font-bold hover:text-indigo-700 hover:underline">Sign up</Link>
            </p>
